			...
			if(isset($parsed[$block][$selector]['transform'])){
				...
				for($i = 0; $i < $num_values; $i++){
					...
					$newproperties = array(
						...
						'-ms-transform' => array($value),
						'-webkit-transform' => array($value)
					);
					$cssp->insert_properties($newproperties, $block, $selector, null, 'transform');
				}
				// Add transform-origin if present
				if(isset($parsed[$block][$selector]['transform-origin'])){
					$num_values = count($parsed[$block][$selector]['transform-origin']);
					for($i = 0; $i < $num_values; $i++){
						$value = $parsed[$block][$selector]['transform-origin'][$i];
						$newproperties = array(
							'-moz-transform-origin' => array($value),
							'-o-transform-origin' => array($value),
							'-ms-transform-origin' => array($value),
							'-webkit-transform-origin' => array($value)
						);
						$cssp->insert_properties($newproperties, $block, $selector, null, 'transform-origin');
					}
				}

				// Applying the matrix-filter for IE (only IE 8 and lower)
				// If we have width & height defined, then replicate transforms with the help of IE's matrix-filter
				if($browser->engine == 'ie' AND $browser->engine_version < 9){
